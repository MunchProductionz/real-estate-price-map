...
async def get_market_data_from_post_codes_from_geojson(city):
    
    # Get postcodes data
    postcodes_path = os.path.join(os.getcwd(), f"../../frontend/public/data/postcodes_{city}.json")
    ...
    square_meter_prices_path = os.path.join(os.getcwd(), f"../../frontend/public/data/square_meter_prices_{city}.json")
    with open(square_meter_prices_path, "w") as file:
        file.write(json.dumps(square_meter_prices, indent=2))
        file.write("\n")
    
    print(f"Updated {len(postcodes_data['features'])} post codes data in for city {city} successfully to ../../frontend/public/data/postcodes_{city}.json")
    print(f"Saved {len(square_meter_prices.keys())} square meter prices for city {city} to ../../frontend/public/data/square_meter_prices_{city}.json")
    return None
